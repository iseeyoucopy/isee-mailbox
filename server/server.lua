        ...
        if Character then
            ...
            exports.oxmysql:query('SELECT mailbox_id FROM mailboxes WHERE char_identifier = ?', {charidentifier}, function(result)
                if result and #result > 0 then
                    ...
                    TriggerClientEvent("isee-mail:mailboxStatus", src, true, mailboxId, firstName .. " " .. lastName)
                else
                    TriggerClientEvent("isee-mail:mailboxStatus", src, false, nil, firstName .. " " .. lastName)
                end
            end)
        else
...
function updateMailboxInfo(src)
    ...
    if User then
        ...
        if Character then
            ...
            exports.oxmysql:execute('UPDATE mailboxes SET first_name = ?, last_name = ? WHERE char_identifier = ?', {firstName, lastName, charidentifier}, function(affectedRows)
...
function GetPlayerFromMailboxId(mailboxId, callback)
    ...
    for _, playerId in ipairs(players) do
        ...
        if User then
            local Character = User.getUsedCharacter
            if Character then
                exports.oxmysql:query('SELECT mailbox_id FROM mailboxes WHERE char_identifier = ?', {Character.charIdentifier}, function(result)
...
RegisterNetEvent("isee-mail:sendMail")
AddEventHandler("isee-mail:sendMail", function(recipientMailboxId, subject, message)
    ...
    if Character.money >= Config.SendMessageFee then
        ...
        exports.oxmysql:query('SELECT mailbox_id FROM mailboxes WHERE char_identifier = ?', {senderCharId}, function(senderResult)
            if senderResult and #senderResult > 0 then
                ...
                if recipientMailboxId and recipientMailboxId ~= "" and subject and subject ~= "" and message and message ~= "" then
                    ...
                    exports.oxmysql:insert('INSERT INTO mailbox_messages (from_char, to_char, from_name, subject, message, timestamp) VALUES (?, ?, ?, ?, ?, ?)', 
                        ...
                        if inserted then
                            ...
                            GetPlayerFromMailboxId(recipientMailboxId, function(recipientSource)
                                if recipientSource then
                                    ...
                                    TriggerClientEvent("isee-mail:checkMailNotification", recipientSource)
                                else
...
RegisterNetEvent("isee-mail:updateMailboxInfo")
AddEventHandler("isee-mail:updateMailboxInfo", function()
    local _source = source
    updateMailboxInfo(_source)
end)

RegisterNetEvent("isee-mail:checkMailbox")
AddEventHandler("isee-mail:checkMailbox", function()
    local _source = source
    local User = VORPcore.getUser(_source)
    local Character = User.getUsedCharacter
    local charidentifier = Character.charIdentifier
    ...
    exports.oxmysql:query('SELECT mailbox_id FROM mailboxes WHERE char_identifier = ?', {charidentifier}, function(result)
        if result and #result > 0 then
            TriggerClientEvent("isee-mail:mailboxStatus", _source, true, result[1].mailbox_id, firstName .. " " .. lastName)
        else
            TriggerClientEvent("isee-mail:mailboxStatus", _source, false, nil, firstName .. " " .. lastName)
        end
    end)
end)

RegisterNetEvent("isee-mail:checkMail")
AddEventHandler("isee-mail:checkMail", function()
    local _source = source
    local User = VORPcore.getUser(_source)
    local Character = User.getUsedCharacter
    local charidentifier = Character.charIdentifier
        ...
        if result and #result > 0 then
            ...
            exports.oxmysql:execute('SELECT * FROM mailbox_messages WHERE to_char = ?', {recipientMailboxId}, function(mails)
                if mails and #mails > 0 then
                    ...
                    TriggerClientEvent("isee-mail:receiveMails", _source, mails)
                else
...
RegisterNetEvent("isee-mail:registerMailbox")
AddEventHandler("isee-mail:registerMailbox", function()
    local _source = source
    local User = VORPcore.getUser(_source)
    local Character = User.getUsedCharacter
    local charidentifier = Character.charIdentifier
            ...
            if insertId then
                exports.oxmysql:execute('SELECT mailbox_id FROM mailboxes WHERE mailbox_id = ?', {insertId}, function(result)
                    if result and #result > 0 then
                        ...
                        TriggerClientEvent("isee-mail:updateMailboxId", _source, newMailboxId)
                        TriggerClientEvent("isee-mail:registerResult", _source, true, _U('MailboxRegistered'))
                    else
                        TriggerClientEvent("isee-mail:registerResult", _source, false, _U('RegistrationError'))
                    end
                end)
            else
                TriggerClientEvent("isee-mail:registerResult", _source, false, _U('MailboxRegistrationFailed'))
            end
        end)
    else
...
RegisterNetEvent("isee-mail:deleteMail")
AddEventHandler("isee-mail:deleteMail", function(mailId)
    local _source = source
...
RegisterNetEvent("isee-mail:getRecipients")
AddEventHandler("isee-mail:getRecipients", function()
    local _source = source

    exports.oxmysql:query('SELECT mailbox_id, CONCAT(first_name, " ", last_name) AS name FROM mailboxes', {}, function(results)
        if results then
            TriggerClientEvent('isee-mail:setRecipients', _source, results)
        end
    end)
end)
